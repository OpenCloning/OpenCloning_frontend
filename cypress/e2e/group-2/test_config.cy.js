...
describe('test that configuration works', () => {
  ...
  it('Annotate features is not available if enablePlannotate is false, but mapping primers is', () => {
    ...
    // Mapping primers needs no external service, so it is available anyway
    cy.get('li[data-value="MapPrimers"]').should('exist');
  });
  it('Both annotation options are available if enablePlannotate is true', () => {
